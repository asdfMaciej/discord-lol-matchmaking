...
def get_skill_differences(team1, team2):
	...
	for pos in diffs.keys():
		diffs[pos] = rank_to_skill(team1[pos][1], pos) - rank_to_skill(team2[pos][1], pos)
	return diffs

def team_to_text(team1, team2):
	positions = ('top', 'mid', 'jungle', 'adc', 'support')
	emojis = {'top':':top:', 'mid':':heavy_minus_sign:', 'jungle':':evergreen_tree:', 'adc':':gun:', 'support':':syringe:'}
...
class Matchmaking:
	...
	def _matchmake(self, players):
			...
			while iteration_pos:
				if not subs:
					if fill_level in (1, 2):
						break_out = False
						for position, players in sorted(position_dict.items(), key=lambda item : len(item[1]), reverse=True):
							if position == needed_pos or position in established_roles:
								continue
							for player in players:
								if player[2][fill_level-1] == needed_pos and 'sub' not in player[1]:
									player[1] += 'sub'
									subs.append(player)
									break_out = True
								if break_out: break
				...
				for sub_n in range(len(subs)):
					...
					position_dict = search_and_remove(sub, position_dict, needed_pos)
					subs.pop(sub_n)
		...
		for position, players in position_dict.items():
			first = True
			for p in players:
				if first: team_1[position] = p
				else: team_2[position] = p 
				first = False

		while True:
			team_diff = (team_to_skill(team_1) - team_to_skill(team_2)) / 2  # needed for swaps
...
class DBHandler:
	...
	async def add_player(self, handle, nick, rank, preferred_pos, secondary_pos):
		...
		if not ids:
			...
			cur.execute('INSERT into players(handle, nick, rank, preferred_pos, secondary_pos, numeric_rank) values (?, ?, ?, ?, ?, ?)', (handle, nick, rank, preferred_pos, secondary_pos, numeric_rank))
			self.db.commit()
			...
			count = 0
			for n_id in ids:
				cur.execute('UPDATE players SET handle=?, nick=?, rank=?, preferred_pos=?, secondary_pos=?, numeric_rank=? WHERE id=?', (handle, nick, rank, preferred_pos, secondary_pos, int(numeric_rank), n_id[0]))
				count += 1
			self.db.commit()
			self.phandler.set_players(self.list_to_format(self.get_players()))
			await self.phandler.msg(
	...
	def get_players(self, order=None):
		...
		txt = "SELECT * FROM players ORDER BY "+orders[order]+" asc;"  # using ? doesn't work, what the hell
		cur = self.db.cursor()
		cur.execute(txt)
		playas = cur.fetchall()
		return playas
	...
	def check_by_handle_or_nick(self, handle, nick):
		cur = self.db.cursor()
		cur.execute("SELECT id from players WHERE handle=? OR nick=?", (handle, nick))
...
class PeopleHandler:
	def __init__(self, client):
		...
		self.slang = {
			'adc':'adc','support':'support','mid':'mid','jungle':'jungle',
			'top':'top','middle':'mid', 'bot':'adc', 'sup':'support',
			'jgl':'jungle','jg':'jungle', 'supp':'support', 'fill':'fill'
		}
		self.ranks_ordered = (
	...
	async def add(self, ctx, *args):
		...
		if len(args) != 4:
			...
			return
		if not re.match(r"<@[!]?[0-9]*>", args[0]):  # the first regex i've ever wrote x D
		...
		handle = args[0].split('<@')[1].split('>')[0] if not '!' in args[0] else args[0].split('!')[1].split('>')[0]
		nick = args[1]
		if 'fill' == args[2]:
			pos1 = 'fill'
			pos2 = 'fill'
	...
	async def lobby(self, ctx, *args):
		...
		try:
			...
			for arg in args:
				if not re.match(r"<@[!]?[0-9]*>", arg):
					...
					ids.append(arg.split('<@')[1].split('>')[0] if not '!' in arg else arg.split('!')[1].split('>')[0])
		...
		if len(ids) != 10:
			...
			return
		hehe = self.db.custom_query("SELECT * from players where handle=?"+" OR handle=?"*9, ids)
		if len(hehe) != 10:
	...
	async def list(self, ctx, *args):
		...
		for gr in chunks(form, 5):
			...
			for player in gr:
				...
				txt += template.format(
					str(player[1]), "<@"+str(player[0])+">", r, str(player[2])
				)
	...
	async def delete(self, ctx, *args):
		...
		if re.match(r"<@[!]?[0-9]*>", args[0]):
			key = "handle"
			asdf = args[0].split('<@')[1].split('>')[0] if not '!' in args[0] else args[0].split('!')[1].split('>')[0]
		else:
			key = "nick"
			asdf = args[0]
...
client = Bot(description="Zajebisty bot#1665", command_prefix="$", pm_help = True)
